...
RC index_btree::init(uint64_t part_cnt) {
    ...
    cur_leaf_per_thd = new bt_node ** [g_num_worker_threads];
    cur_idx_per_thd = new uint32_t * [g_num_worker_threads];
    for (uint32_t i = 0; i < g_num_worker_threads; i ++) {
        cur_leaf_per_thd[i] = new bt_node *;
        cur_idx_per_thd[i] = new uint32_t;
    }
...
bool index_btree::index_exist(uint64_t key) {
...
RC index_btree::index_next(uint64_t thd_id, itemid_t * &item, bool samekey) {
    ...
    uint64_t cur_key = leaf->keys[idx] ;
...
RC index_btree::index_read(uint64_t key, itemid_t *& item) {
    assert(false);
    return RCOK;
}

RC
index_btree::index_read(uint64_t key,
    itemid_t *& item,
    int part_id) {
...
RC index_btree::index_insert(uint64_t key, itemid_t * item, int part_id) {
    glob_param params;
...
RC index_btree::make_node(uint64_t part_id, bt_node *& node) {
    ...
    new_node->keys = new uint64_t[order - 1];
...
RC index_btree::start_new_tree(glob_param params, uint64_t key, itemid_t * item) {
    RC rc;
...
RC index_btree::find_leaf(glob_param params, uint64_t key, idx_acc_t access_type, bt_node *& leaf) {
    bt_node * last_ex = NULL;
...
RC index_btree::find_leaf(glob_param params, uint64_t key, idx_acc_t access_type, bt_node *& leaf, bt_node  *& last_ex)
{
    uint32_t i;
...
RC index_btree::insert_into_leaf(glob_param params, bt_node * leaf, uint64_t key, itemid_t * item) {
...
RC index_btree::split_lf_insert(glob_param params, bt_node * leaf, uint64_t key, itemid_t * item) {
    RC rc;
...
RC index_btree::insert_into_new_root(
    glob_param params, bt_node * left, uint64_t key, bt_node * right)
{
    RC rc;
    uint64_t part_id = params.part_id;
    bt_node * new_root;
...
int index_btree::leaf_has_key(bt_node * leaf, uint64_t key) {
    for (uint32_t i = 0; i < leaf->num_keys; i++)
        if (leaf->keys[i] == key)
            return i;
    return -1;
