...
class LimitDepthScheduler : public Scheduler {
public:
  LimitDepthScheduler(Workload * wl, int maxSteps) : Scheduler(wl, maxSteps) {
    scheduler_name = "LimitDepth Scheduler";
    ...
    int tryLock(int key, int txn_id, int type) {
      ...
      for (int i = 0; i < numTxns; i ++) {
        ...
        if (conflict(table[key][i], type)) {
          if (is_waiting[i]) // max wait depth = 1, cannot wait for a waiting txn.
            return 0;
          else
            ret = 2;
        }
